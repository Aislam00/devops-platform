...
resource "aws_secretsmanager_secret_version" "database_connection" {
  ...
  secret_string = jsonencode({
    DATABASE_URL      = "postgresql://${var.db_master_username}:${urlencode(var.db_password)}@${split(":", var.db_instance_endpoint)[0]}:${var.db_instance_port}/${var.db_name}?sslmode=require"
